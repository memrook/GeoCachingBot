...
func (b *Bot) handleUserMessage(message *tgbotapi.Message) {
	...
	// Если нет активной сессии и это текстовое сообщение, ищем кэш по кодовому слову
...
func (b *Bot) handleCacheSearch(userID int64, codeWord string) {
	...
	instruction := fmt.Sprintf(`🎯 Кэш найден: %s
...
4️⃣ Установите время: %d час(а)
5️⃣ Нажмите "Поделиться"

⚠️ Важно: нужна именно ТРАНСЛЯЦИЯ, а не обычная геопозиция!`,
...
func (b *Bot) handleTargetReached(userID int64, cache *Cache) {
	...
	congratsMsg := fmt.Sprintf("🎉 Поздравляем! Вы нашли кэш: %s\n\n📷 Вот фотография места:\n\n💡 Вы можете остановить передачу геолокации и начать поиск нового кэша!", cache.CodeWord)
	...
	photoMsg.Caption = "🏆 Поиск завершен! Введите новое кодовое слово для следующего кэша."
